<?php
// /public_html/main/blocklist/api/remove_whitelist.php
...
$removed_dns = [];
$removed_tls = [];
$removed_overrides = []; // analogous to sinkholes in block removal
...
try {
  ...
  if ($address !== '') {
    ...
    if (is_ip($address)) {
      // 1) Remove FW rows (whitelist) for this IP + collect hosts from comments
      foreach ($fwRows as $r) {
        if (($r['list'] ?? '') === 'whitelist' && ($r['address'] ?? '') === $address) {
          $rid = row_id($r);
          if ($rid) {
      ...
      foreach ($dnsRows as $r) {
        ...
        if ($match) {
          ...
          push_unique($removed_overrides, $label);
    ...
    elseif (is_hostname($address)) {
      ...
      // 2) Remove FW rows whose comment references the host(s) — list is 'whitelist'
      foreach ($fwRows as $r) {
        if (($r['list'] ?? '') !== 'whitelist') continue;
        $rid = row_id($r);
        if (!$rid) continue;
        $cmt = strtolower($r['comment'] ?? '');
