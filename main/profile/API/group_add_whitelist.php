<?php
// Group-scoped WHITELIST add:
// - address-list: whitelist_<group>_ip   (IPv4 / resolved A records)
// - filter: ACCEPT tcp with tls-host + src-address-list=profile:<group>
// No DNS static is ever touched for whitelist.
...
$listWhite = "whitelist_{$group}_ip";
...
$acceptExists = function(string $name, string $srcList) use ($fwFlt): bool {
  foreach ($fwFlt as $r){
    if (
...
$added_tls_accept=[]; $skipped_tls_accept=[];
$debug=[];

try{
  if (is_ip($input) || is_cidr($input)) {
    if (!$addrExists($listWhite, $input)) {
      $api->talk('/ip/firewall/address-list/add', [
        'list'    => $listWhite,
        'address' => $input,
        'comment' => "group:$group ".($comment ?: '')
      ]);
      $added_ips[]=$input; $debug[]="addrlist add $listWhite $input";
    } else { $skipped_ips[]=$input; $debug[]="addrlist exists $listWhite $input"; }
  }
  ...
  if (is_hostname_or_url($input)) {
    ...
    foreach ($ips as $ip) {
      if (!$addrExists($listWhite, $ip)) {
        $api->talk('/ip/firewall/address-list/add', [
          'list'    => $listWhite,
          'address' => $ip,
          'comment' => "group:$group From: $dom"
        ]);
        $added_ips[]=$ip; $debug[]="addrlist add $listWhite $ip (From $dom)";
      } else { $skipped_ips[]=$ip; $debug[]="addrlist exists $listWhite $ip"; }
    }

    foreach ($names as $n) {
      if (!$acceptExists($n,$srcList)) {
        ...
        $added_tls_accept[]=$n; $debug[]="tls accept add name=$n src=$srcList";
      } else { $skipped_tls_accept[]=$n; $debug[]="tls accept exists name=$n src=$srcList"; }
    }
  }
  ...
  jexit([
    'ok'=>true,
    'message'=>'Whitelisted (group-scoped)',
    'group'=>$group,
    'input'=>$input,
    'added_ips'=>$added_ips,
    'skipped_ips'=>$skipped_ips,
    'added_tls_accept'=>$added_tls_accept,
    'skipped_tls_accept'=>$skipped_tls_accept,
    'debug'=>$debug
  ]);
